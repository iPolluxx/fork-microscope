...
function bootstrap(){
  ...
  function renderEditor(){
    ...
    $('prompt-list').replaceChildren(...current.prompts.map((p,index)=>{
      ...
      card.append(labeled('Prompt title',title),labeled('Prompt text',prompt),labeled('Answer labels · one per line',answers));
      ...
      const pair=document.createElement('div');pair.className='field-pair';pair.append(labeled('Original-response token cap',makeInput(`cap-${p.id}`,'number',p.max_tokens,{min:8,max:4096,step:1},value=>updateModel(p,'max_tokens',value))),labeled('Original-response seed',makeInput(`seed-${p.id}`,'number',p.seed,{min:0,max:2147483647,step:1},value=>updateModel(p,'seed',value))));advancedBody.append(pair);advanced.append(advancedBody);card.append(advanced);
  ...
  function updateActions(){
    ...
    $('save-set').disabled=writePending||(saved&&!changed);$('delete-set').disabled=writePending||!saved;$('add-prompt').disabled=writePending||current.prompts.length>=50;
    ...
    $('start-batch').disabled=Boolean(reason);$('batch-readiness').textContent=reason||`${count} ${count===1?'prompt':'prompts'} ready. The worker will generate a separate trace for each.`;$('scan-fields').disabled=writePending;
  ...
  async function saveSet(){
    if(writePending)return;const issue=validatePromptSet(current);if(issue){signal('set-error',issue);return;}
  ...
  async function importSet(file){
    ...
    try{if(file.size>2*1024*1024)throw new Error('Prompt-set imports must be no larger than 2 MB.');let data;try{data=JSON.parse(await file.text());}catch{throw new Error('That file is not valid JSON.');}const payload=parsePromptSetImport(data);if(requestEpoch!==epoch)return;const value=await api('prompt-set',payload);if(requestEpoch!==epoch)return;if(!value.set?.id)throw new Error('The worker did not confirm the imported set. Refresh saved sets before retrying.');sets=[value.set,...sets.filter(s=>s.id!==value.set.id)];openSet(value.set);signal('workspace-notice','Imported as a new saved prompt set. No model execution was started.');
  ...
  $('add-prompt').onclick=()=>{if(current.prompts.length>=50)return;const p=makePrompt(current.prompts.length+1);current.prompts.push(p);selected.add(p.id);renderEditor();$(`title-${p.id}`).focus();};
