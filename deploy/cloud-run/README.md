...
- `/healthz` returns `ok`; `/api/live/status` returns 404 because compute belongs on the separately owned worker.
...
Deployment verification is recorded in [the hosting report](../../reports/gcp-hosting/README.md). A hosted website does not change the Git repository or GPU image's publication status.
