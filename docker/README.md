...
The image contains the CUDA environment, dashboard, worker and pinned upstream checkout. It starts **without a selected model and without downloading weights**. Use Configure to inspect and attach a native model. The current source includes the hosted-worker, saved-prompt and comparison workflow; the older published image does not contain these changes.
...
Choose hardware for your model and continuation context. The earlier Muse profile ran on an A100 80GB; this is evidence for that profile, not a universal hardware requirement. Native model eligibility and weight-memory estimates are available in Configure. The local `fork-microscope:public-workflow` build passed 146 Python and 10 JavaScript tests on CPU, plus authenticated startup checks. It has not had a new GPU acceptance run.
...
Mount `/workspace` if you want these to survive replacing the container. Otherwise export evidence and prompt sets in the dashboard, or copy both data directories before terminating a disposable VM. Cache download, completed evidence and unexecuted drafts are distinct artifacts.

## Distribution status

The pinned Goodfire checkout has no LICENSE file. [THIRD-PARTY.md](../THIRD-PARTY.md) records this unresolved public-redistribution question. The previous package remains private; these changes do not change repository/image visibility or publish a new image. The separately built static dashboard does not bundle the upstream Python code or datasets.

Previous build tags and historical measurements are retained in [container history](../reports/container-history.md). They are not current default settings or validation for this source revision.

The optional container publishing workflow requires an existing **private** GHCR package before it pushes. It intentionally fails for a missing or public package while bundled-upstream permissions remain unresolved. Local builds do not require a registry.
